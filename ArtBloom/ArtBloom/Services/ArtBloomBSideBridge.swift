...
@MainActor
protocol ArtBloomBSideBridgeHost: AnyObject {
...
final class ArtBloomBSideBridge: NSObject, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate, PHPickerViewControllerDelegate {
    static let messageName = "syncAppInfo"
    private static let bridgeVersion = "2026-07-09.2"
    #if DEBUG
    ...
    #endif
    private static let toastViewTag = 92_067_501

    private weak var host: ArtBloomBSideBridgeHost?
    private var pendingPhotoRequestId: String?
    private weak var activePaymentBrowser: ArtBloomBSidePaymentController?
    private var activePaymentBrowserContext: [String: Any]?
    private var activePaymentRequestId: String?

    init(host: ArtBloomBSideBridgeHost) {
        self.host = host
        super.init()
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handlePushPayloadNotification(_:)),
            name: .artBloomBSidePushPayloadReceived,
            object: nil
        )
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handlePushTokenNotification(_:)),
            name: .artBloomBSidePushTokenUpdated,
            object: nil
        )
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handlePaymentCallbackNotification(_:)),
            name: .artBloomBSidePaymentCallbackReceived,
            object: nil
        )
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(handlePaymentTransactionNotification(_:)),
            name: .artBloomBSidePaymentTransactionUpdated,
            object: nil
        )
    }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case "getCachedVideoURL":
            ...
            let cachedURL = ArtBloomBSideMediaCache.shared.displayURL(for: urlString, mediaType: "video")
            respond(requestId: requestId, result: [
        ...
        case "prefetchVideo":
            let urlString = Self.stringParam("url", from: body)
            Task {
                _ = await ArtBloomBSideMediaCache.shared.prefetch(remoteURLString: urlString, mediaType: "video")
                let cachedURL = ArtBloomBSideMediaCache.shared.displayURL(for: urlString, mediaType: "video")
                await MainActor.run {
        ...
        case "getTemplateFeedCache":
            ...
            respond(requestId: requestId, result: ArtBloomBSideJSONCache.shared.value(namespace: "templateFeed", key: key))
        case "setTemplateFeedCache":
            ...
            let saved = ArtBloomBSideJSONCache.shared.setValue(namespace: "templateFeed", key: key, value: value, ttlSeconds: ttl)
            respond(requestId: requestId, result: ["saved": saved])
        case "getTemplateDetailCache":
            let key = Self.stringParam("key", from: body)
            respond(requestId: requestId, result: ArtBloomBSideJSONCache.shared.value(namespace: "templateDetail", key: key))
        case "setTemplateDetailCache":
            let params = Self.params(from: body)
            let key = params["key"] as? String ?? ""
            let value = params["value"] ?? NSNull()
            let ttl = params["ttlSeconds"] as? TimeInterval
            let saved = ArtBloomBSideJSONCache.shared.setValue(namespace: "templateDetail", key: key, value: value, ttlSeconds: ttl)
            respond(requestId: requestId, result: ["saved": saved])
        #endif
        case "logAnalyticsEvent":
            ...
            Task {
                let result = await ArtBloomAFManager.shared.logEvent(
                    channelId: channel,
                    eventName: eventName,
                    values: eventValues
                )
        ...
        case "getAppInfo":
            var result: [String: Any] = [
                "platform": "ios",
                "appName": ArtBloomBSideConfig.appDisplayName,
                "appVersion": ArtBloomBSideConfig.appVersion,
                "version": ArtBloomBSideConfig.appVersion,
                "systemVersion": UIDevice.current.systemVersion,
                "systemLocale": Locale.current.identifier,
                "appLanguage": AppLanguage.load().rawValue,
            ...
            result["resBaseURL"] = ArtBloomBSideConfig.effectiveResBaseURL
            result["channel"] = ArtBloomBSideConfig.channel
            #if DEBUG
            result["buildConfiguration"] = ArtBloomBSideConfig.buildConfigurationLabel
        ...
        case "prepareLoginAttribution":
            let channel = Self.stringParam("channel", from: body)
            Task {
                let result = await ArtBloomAFManager.shared.prepareLoginAttribution(channelId: channel)
                await MainActor.run {
                    self.respond(requestId: requestId, result: result)
                }
            }
        case "markLoginCompleted":
            ArtBloomAFManager.shared.markLoginCompleted()
        ...
        case "getCachedMediaURL":
            ...
            let cachedURL = ArtBloomBSideMediaCache.shared.displayURL(for: urlString, mediaType: mediaType)
            respond(requestId: requestId, result: [
                "url": cachedURL?.absoluteString ?? NSNull(),
                "cached": cachedURL != nil
            ])
        case "prefetchMedia":
            ...
            let mediaType = Self.stringParam("mediaType", from: body)
            Task {
                _ = await ArtBloomBSideMediaCache.shared.prefetch(remoteURLString: urlString, mediaType: mediaType)
                let cachedURL = ArtBloomBSideMediaCache.shared.displayURL(for: urlString, mediaType: mediaType)
                await MainActor.run {
                    self.respond(requestId: requestId, result: [
                        "url": cachedURL?.absoluteString ?? NSNull(),
        ...
        case "saveMediaToAlbum":
            ...
            Task {
                do {
                    ...
                    let code = nsError.domain == "ArtBloomAlbum" && nsError.code == -2
        ...
        case "getJSONCache":
            ...
            let key = params["key"] as? String ?? ""
            respond(requestId: requestId, result: ArtBloomBSideJSONCache.shared.value(namespace: namespace, key: key))
        case "setJSONCache":
            let params = Self.params(from: body)
            let namespace = params["namespace"] as? String ?? "default"
            let key = params["key"] as? String ?? ""
            let value = params["value"] ?? NSNull()
            let ttl = params["ttlSeconds"] as? TimeInterval
            let saved = ArtBloomBSideJSONCache.shared.setValue(namespace: namespace, key: key, value: value, ttlSeconds: ttl)
            respond(requestId: requestId, result: ["saved": saved])
        case "pickPhoto":
            ...
            presentPhotoPicker()
        case "clearCache":
            let jsonFiles = ArtBloomBSideJSONCache.shared.clearAll()
            let mediaFiles = ArtBloomBSideMediaCache.shared.clearAll()
            respond(requestId: requestId, result: [
        ...
        case "registerPush":
            ArtBloomPushManager.shared.register { [weak self] result in
                DispatchQueue.main.async {
                    self?.respond(requestId: requestId, result: result)
                }
            }
        case "getLaunchPushPayload":
            respond(requestId: requestId, result: ArtBloomPushManager.shared.consumeLaunchPayload())
        case "openPayment":
            let urlString = Self.stringParam("url", from: body)
            ...
            if type == "apple_pay" && urlString.isEmpty {
                let params = Self.params(from: body)
                Task {
                    let result = await ArtBloomPaymentManager.shared.purchase(params: params)
                    await MainActor.run {
                        self.respond(requestId: requestId, result: result)
                    }
                }
        ...
        case "restorePayment":
            Task {
                let result = await ArtBloomPaymentManager.shared.restoreTransactions()
                await MainActor.run {
                    self.respond(requestId: requestId, result: result)
                }
            }
        case "finishPaymentTransaction":
            ...
            Task {
                let result = await ArtBloomPaymentManager.shared.finishTransaction(transactionId: transactionId)
                await MainActor.run {
                    self.respond(requestId: requestId, result: result)
                }
            }
        ...
        case "closeNative", "switchToNative":
            host?.switchToNative()
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        host?.navigationFinished()
    }

    func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
        host?.failWebLoad(error.localizedDescription)
    }
    ...
    func webView(
        ...
        let allowedSchemes = ["http", "https", "about", "blob", "data", ArtBloomBSideMediaCacheSchemeHandler.scheme]
    ...
    private static func handlePaymentReturnURL(_ url: URL) -> Bool {
        guard ArtBloomPaymentRedirectReturnURL.matches(url) else { return false }
    ...
    private func presentPaymentBrowser(url: URL, params: [String: Any], requestId: String) -> Bool {
        ...
        let browser = ArtBloomBSidePaymentController(url: url) { [weak self] in
    ...
    private func presentBrowserWebView(url: URL, title: String) -> Bool {
        guard let presenter = Self.topViewController() else { return false }
        let browser = ArtBloomBSidePageController(
            url: url,
            title: title.isEmpty ? ArtBloomBSideConfig.appDisplayName : title
        )
        browser.modalPresentationStyle = .fullScreen
        presenter.present(browser, animated: true)
        return true
    ...
    private static func saveMediaToAlbum(urlString: String, mediaType: String, fileName: String) async throws -> [String: Any] {
        guard !urlString.isEmpty, let url = URL(string: urlString) else {
            throw NSError(domain: "ArtBloomAlbum", code: -1, userInfo: [NSLocalizedDescriptionKey: "Media URL is invalid"])
        }

        let authorized = await requestPhotoAddPermission()
        guard authorized else {
            throw NSError(domain: "ArtBloomAlbum", code: -2, userInfo: [NSLocalizedDescriptionKey: "Photo library permission denied"])
        }
        ...
        if resolvedType == "video" {
            ...
            guard requested else {
                throw NSError(domain: "ArtBloomAlbum", code: -3, userInfo: [NSLocalizedDescriptionKey: "Unable to save video"])
            }
        ...
        guard let image = UIImage(data: data) else {
            throw NSError(domain: "ArtBloomAlbum", code: -4, userInfo: [NSLocalizedDescriptionKey: "Unable to read image"])
        }
        try await PHPhotoLibrary.shared().performChanges {
    ...
    private static func mediaData(from url: URL, originalURLString: String, mediaType: String) async throws -> Data {
        ...
        if let httpResponse = response as? HTTPURLResponse,
           !(200..<300).contains(httpResponse.statusCode) {
            throw NSError(domain: "ArtBloomAlbum", code: -5, userInfo: [NSLocalizedDescriptionKey: "Media download failed"])
        }
        return data
    }
    ...
    private static func localMediaURL(for url: URL, originalURLString: String, mediaType: String, fileName: String) async throws -> (url: URL, shouldCleanup: Bool) {
        ...
        if !remoteURLString.isEmpty,
           let prefetchedURL = await ArtBloomBSideMediaCache.shared.prefetch(remoteURLString: remoteURLString, mediaType: mediaType) {
    ...
    private static func cachedMediaURLForAlbum(url: URL, originalURLString: String, mediaType: String) -> URL? {
        ...
        return ArtBloomBSideMediaCache.shared.cachedURL(for: remoteURLString, mediaType: mediaType)
    }

    private static func cacheSourceURLString(from url: URL, fallback: String) -> String {
        if url.scheme == ArtBloomBSideMediaCacheSchemeHandler.scheme,
