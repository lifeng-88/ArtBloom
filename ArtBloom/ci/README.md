...
| `APP_STORE_CONNECT_API_PRIVATE_KEY` | `.p8` 全文（含 `BEGIN/END PRIVATE KEY`） |
...
- CI 运行 `ci/prepare_ci_signing.py` 写入 `DEVELOPMENT_TEAM` 并强制 Automatic Signing
- CI 运行 `ci/ensure_ci_device.py` 自动注册占位设备（Development 描述文件必需）
